...
Panel {
  ...
  // Per-model request counts, when a collector can source them. Ollama Cloud
  ...
  function limitWindow(label, percent, resetAt, title, entry) {
    return {
      title: String(title || "") !== "" ? String(title) : windowTitle(label),
      percent: Number(percent),
      resetAt: String(resetAt || ""),
      // A countdown a collector derived rather than read from the provider:
      // shown with a "~" so it never reads as more exact than it is, or with
  ...
  // Request counts per model, from the limit window a collector marked up.
  ...
  function requestModelRows(p) {
    ...
    for (var i = 0; i < list.length; i++) {
      var entry = list[i] || {}
      var marked = entry.requestModels || []
      if (marked.length === 0) continue
      // "This week" is the window that says so; failing that, whichever
      // marked-up window there is (a provider may only expose a session one).
      if (String(entry.title || "").toLowerCase() === "weekly") { rows = marked; break }
      if (rows === null) rows = marked
    }
    if (rows === null) return []
    var out = []
    for (var j = 0; j < rows.length; j++) {
      var row = rows[j] || {}
      var count = Number(row.requests)
      if (!(count > 0)) continue
      out.push({ name: usage.friendlyModelName(String(row.name || "")), total: count })
    }
    out.sort(function(a, b) { return b.total - a.total })
    return out.slice(0, 4)
  }

  // Numbers with thousands separators, without leaning on the font or the
  // locale to do it.
  ...
  function asOfText() {
    ...
    for (var i = 0; i < root.limits.length; i++) {
      ...
      var why = String(window.staleReason || "")
      if (why === "auth") staleReason = "auth"
      else if (why === "unrefreshed" && staleReason === "") staleReason = "unrefreshed"
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // ---------- Models by requests ----------
          PanelSeparator {
            visible: requestSection.visible
            foreground: root.foreground
          }
          ...
          Column {
            ...
            Repeater {
              model: root.requestModels

              ModelRow {
                required property var modelData
                width: requestSection.width
                row: modelData
                share: modelData.total / Math.max(1, root.requestModels[0].total)
                // Requests, not tokens: the row still scales to its heaviest
                // model, but the number means "times asked", so it says so.
                valueText: root.formatCount(modelData.total)
                tooltip: root.formatCount(modelData.total) + " requests"
                  + " · counted by the provider for this week's window"
              }
            }
          }
  ...
  component LimitRow: Column {
    ...
    Item {
      ...
      Text {
        ...
        textFormat: Text.PlainText
        text: {
          var p = limitRow.window ? limitRow.window.percent : -1
          if (p < 0) return "—"
          // Always one decimal so 18.9% / 73.8% match the provider dashboards.
          // Integer percents render as 68.0% rather than rounding away the tenths.
    ...
    Text {
      ...
      text: {
        ...
        if (limitRow.window && limitRow.window.provisional) prefix = "Resets in ≤"
        else if (limitRow.window && limitRow.window.estimated) prefix = "Resets in ~"
        return prefix + root.formatDuration(remainingMs)
      }
      color: root.dim
      font.family: root.fontFamily
      font.pixelSize: Style.font.caption
  ...
  component ModelRow: Item {
    ...
    // Rows that count something else than tokens hand their own text and
    // hover line in, so the component stays one shape.
    ...
    Text {
      id: modelTokens
      textFormat: Text.PlainText
      text: modelRow.row ? (modelRow.valueText !== "" ? modelRow.valueText : usage.formatTokenCount(modelRow.row.total)) : ""
      color: root.dim
      font.family: root.fontFamily
      font.pixelSize: Style.font.bodySmall
      font.bold: true
    ...
    PanelToolTip {
      ...
      text: modelRow.tooltip !== "" ? modelRow.tooltip : root.modelTooltip(modelRow.row)
      fontFamily: root.fontFamily
    }
  }
}
